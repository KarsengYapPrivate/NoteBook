import { _decorator, Component, instantiate, Label, Node, sp, tween, UITransform, v3 } from 'cc';
...
import { Reel } from 'db://assets/SlotsCore/Scripts/Core/Reel';
...
@ccclass('GalacticWinPresenter')
export class GalacticWinPresenter extends PresenterPrototype {
    ...
    private multiplierResumePrsenter = false;
    private winAmount = 0;

    private spawnSymbol = 0;
    ...
    async DestroySymbol(result) {
        ...
        for (let i = 0; i < reels.length; i++) {
            reels[i].node.getComponent(Reel).ChangeSymbolLayer();
        }

        await Utils.WaitForSeconds(0.1);
        if(result.winnings <= 0) {
            if(this.resultIndex > 0) {
                this.multiplierResumePrsenter = false;
    
                let detail = {
                    index : this.resultIndex,
                    result : result.p01MultiplierArray
                }
    
                dispatchEvent(new CustomEvent('multiplier' , {detail : detail}))
            }
    
            await Utils.WaitForCondition((()=>this.multiplierResumePrsenter).bind(this))
        ...
        GalacticColoniesConfig.instance.characterGroup.leftCharacter.getComponent(sp.Skeleton).animation = 'winning'; 
        GalacticColoniesConfig.instance.characterGroup.rightCharacter.getComponent(sp.Skeleton).animation = 'winning';
        // GalacticColoniesConfig.instance.characterGroup.leftCharacter.getComponent(sp.Skeleton).setCompleteListener((function(data) {
        //     if(data.animation.name == 'winning') {
        //         GalacticColoniesConfig.instance.characterGroup.leftCharacter.getComponent(sp.Skeleton).animation = 'idle';
        //         GalacticColoniesConfig.instance.characterGroup.rightCharacter.getComponent(sp.Skeleton).animation = 'idle';
        //     }
        // }).bind(this))
        ...
        for (let i = 0; i < result.destroySymbolIndexes.length; i++) {
            for (let j = 0; j < result.destroySymbolIndexes[i].length; j++) {
                if(result.destroySymbolIndexes[i][j] == 0) {
                    ...
                    reels[i].node.children[j].getComponent(CacasdingSymbol).Dim();
                }
            }
        }

        AudioManager.instance.PlayEventSound(GalacticColoniesConfig.instance.sound.secondSymbolWin);

        await Utils.WaitForCondition((()=>this.destroyDone).bind(this))

        GalacticColoniesConfig.instance.winningNode.lineWinNode.active = true;
        this.winAmount = this.winAmount + result.winnings;
        AudioManager.instance.PlayEventSound(GalacticColoniesConfig.instance.sound.coinCountLoop);
        TweenUtils.FadeUIOpacity(GalacticColoniesConfig.instance.winningNode.lineWinNode , 0.125 , 255).then((()=>{
            GalacticColoniesConfig.instance.winningNode.lineWinNode.getComponentInChildren(RollingNumber).SetTarget(this.winAmount , 1.5 , 0 , (()=>{
    ...
    async MoveSymbol() {
        ...
        for(let i = 0; i < reels.length; i++) {
            for(let j = 0; j < reels[i].node.children.length; j++) {
                reels[i].node.children[j].getComponent(CacasdingSymbol).Shine();
            }
        }
        ...
        if(this.liveSymbolMove == 0) {
            this.DropSymbol();
        }

        GalacticColoniesConfig.instance.winningNode.wayNumberNode.getComponent(RollingNumber).SetTarget(GameData.instance.GetResult().data[this.resultIndex].ways , this.spawnSymbol * 0.2)
    }
    ...
    async SpawnSymbol(reel , result , index) {
        switch(reel.node.getComponent(GalacticReelDropController).direction) {
            case Direction.Up : {
                ...
                for (let j = newReelsSymbolNumber - 1; j >= 0; j--) {
                   let symbol = instantiate(reel.node.getComponent(GalacticReelDropController).symbolPrefab);
                ...
                for (let k = 0; k < reel.node.children.length; k++) {
                    ...
                    let symbolHeight = reel.node.getComponent(GalacticReelDropController).symbolHeight;
                    ...
                    let numberOfSymbols = reel.node.getComponent(GalacticReelDropController).numberOfSymbols
                    ...
                    if(!reel.node.children[k].getComponent(CacasdingSymbol).liveSymbol) {
                        ...
                        this.MoveLiveSymbolY(symbol , reel.node.getComponent(GalacticReelDropController).direction);
                    }
                }     
                break;
            }
            case Direction.Down : {
                let newReelsSymbolNumber = result[index].length - reel.node.children.length;
                this.spawnSymbol = this.spawnSymbol + newReelsSymbolNumber;
                for (let j = newReelsSymbolNumber - 1; j >= 0; j--) {
                   let symbol = instantiate(reel.node.getComponent(GalacticReelDropController).symbolPrefab);
                   reel.node.addChild(symbol);
            ...
            case Direction.Left : {
                ...
                for (let k = 0; k < reel.node.children.length; k++) {
                    this.totalSymbolMove++;
                    let symbolWidth = reel.node.getComponent(GalacticReelDropController).symbolWidth;
                    let symbol = reel.node.children[k];
                    ...
                    if(!reel.node.children[k].getComponent(CacasdingSymbol).liveSymbol) {
                        ...
                        this.MoveLiveSymbolX(symbol , reel.node.getComponent(GalacticReelDropController).direction);
                    }
                }
    ...
    async DropSymbol() {
        // GalacticColoniesConfig.instance.winningNode.wayNumberNode.getComponent(RollingNumber).SetTarget(GameData.instance.GetResult().data[this.resultIndex].ways , 1);

        let reels = ReelController.instance.reelNormals;

        for(let i = 0; i < reels.length; i++) { 
            switch(reels[i].node.getComponent(GalacticReelDropController).direction) {
                case Direction.Up : {
                    for (let k = 0; k < reels[i].node.children.length; k++) {
                        let numberOfSymbols = reels[i].node.getComponent(GalacticReelDropController).numberOfSymbols;
                        let symbolDropIntervel = reels[i].node.getComponent(GalacticReelDropController).symbolDropIntervel;
                        let symbol = reels[i].node.children[k];
                        if (!reels[i].node.children[k].getComponent(CacasdingSymbol).liveSymbol) {
                            this.MoveLiveSymbolY(symbol , reels[i].node.getComponent(GalacticReelDropController).direction);
                ...
                case Direction.Left : {
                    for (let k = 0; k < reels[i].node.children.length; k++) {
                        ...
                        let symbol = reels[i].node.children[k];
                        if (!reels[i].node.children[k].getComponent(CacasdingSymbol).liveSymbol) {
                            this.MoveLiveSymbolX(symbol , reels[i].node.getComponent(GalacticReelDropController).direction);
                            await Utils.WaitForSeconds(symbolDropIntervel - ((symbolDropIntervel / (numberOfSymbols + 2)) * (reels[i].node.children.length - k)))
                        }
                    }  
    ...
    ResumePresenter() {
        this.multiplierResumePrsenter = true;
    }
}
