// README
// apply_jsb is yet to be updated for v3
// currently only 
    ...
    cpp_str = `bool FileUtils::init()
{
	...
	setDecryptKeyAndSign("tempEncryptSign","tempEncryptKey");
	return true;
}

void FileUtils::setDecryptKeyAndSign(std::string sign, std::string key)
{
	this->decryptSign = sign;
	this->decryptKey = key;
}
...
std::string FileUtils::getStringFromFile(const std::string& filename)
{
	...
	if (s.length() > decryptSign.size() && s.find(decryptSign.c_str()) == 0) {
		s.erase(0, decryptSign.size());
		int kindex = 0;
		for (int i = 0; i < s.length(); i++) {
			if (kindex >= decryptKey.size()) kindex = 0;
			s[i] ^= decryptKey[kindex];
			kindex++;
		}
	}
	return s;
...
Data FileUtils::getDataFromFile(const std::string& filename)
{
	...
	if (d.getSize() > 4) {
		...
		if (memcmp(bytes, decryptSign.c_str(), decryptSign.size()) == 0) {
			ssize_t realSize = d.getSize() - decryptSign.size();
			unsigned char* data = (unsigned char*)calloc(1, realSize);
			memcpy(data, bytes + decryptSign.size(), realSize);
			int kindex = 0;
			for (int i = 0; i < realSize; i++) {
				if (kindex >= decryptKey.size()) kindex = 0;
				data[i] ^= decryptKey[kindex];
				kindex++;
			}
...
h_str = `virtual void valueVectorCompact(ValueVector& valueVector);
	...
	std::string decryptSign = "sign";
	std::string decryptKey = "key";

	public:
	void setDecryptKeyAndSign(std::string sign = "",std::string key = "");
};
...
engine_str = `function _getRealPath(path) {
  ...
  if(!needMixFilename){//tag
	...
	if(name[8]=="-"&&name[13]=="-"&&name[18]=="-"&&name[23]=="-"){
		let md5 = jhec.str_to_md5(name+'tag')
		let arr2 = [8,13,18,23]
...
function checkIsEncrypted(filePath) {
  if(!encryptSign){
      return false;
  }
  let arrbuf = Fs.readFileSync(filePath);

  let signBuf = new Uint8Array(strToBytes(encryptSign));
...
function copyHelper() {
    // let fromPath = Editor.url("packages://jhec-encrypt/panel/md5_util.js","utf-8");
    ...
    let mainJsPath = Path.join(buildFolderPath,"main.js")
    if(!checkIsEncrypted(mainJsPath)){
 ...
 */
 function insertToJsEngineJs() {
    let jsb_adapterPath = Path.join(buildFolderPath,"jsb-adapter");
    let jsEnjinePath = Path.join(jsb_adapterPath,"jsb-engine.js");
  
    if(checkIsEncrypted(jsEnjinePath)){
      return
    }
    ...
    str = str.replace("let md5 = jhec.str_to_md5(name+'tag')",`let md5 = jhec.str_to_md5(name+"${nameMixSign}")`)
...
function insertToFileUtils() {
    ...
    do{
        ...
        if(hStr.indexOf("setDecryptKeyAndSign")>=0){
            break;
        }
    ...
    do{
        let keyLine = `setDecryptKeyAndSign("${encryptSign}","${encryptKey}");`
        ...
        cpp_str = cpp_str.replace('setDecryptKeyAndSign("tempEncryptSign","tempEncryptKey");',keyLine)
...
module.exports = function applyJsb({_buildFolderPath,_encryptSign,_encryptKey,_needMixFilename,_nameMixSign}) {
