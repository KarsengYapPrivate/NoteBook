...
function _getRealPath(path) {
	...
	if(name[8]=="-"&&name[13]=="-"&&name[18]=="-"&&name[23]=="-"){
		let md5 = jhec.str_to_md5(name+nameMixSign)
		let arr2 = [8,13,18,23]
...
function downloadArrayBuffer (url, options, onComplete) {
    ...
    downloadFile(url, options, options.onFileProgress, function (err,data) {
        if(!err){
            data = _decryptTool.decodeArrayBuffer(data);
        }
...
function downloadImage(url, options, onComplete) {
    downloadArrayBuffer(url,options,function(err, data){
        ...
        // edited by JH
        // hardcoded to always use blob on v3 due to the imageBitmap is set to false
        // cc.sys.capabilities.imageBitmap is false for v3 but image built in bitmap
        // using arrayBufferToBase64Img below will cause maximum stack exceeded and unable to decrypt the image correctly
...
class DecryptTool{
    ...
    decodeArrayBuffer(arrbuf,sign=this.encryptSign,key=this.encryptKey){
        ...
        let buf = this.strToBytes(sign);
        let signBuf = new Uint8Array(buf);
...
var _decryptTool = new DecryptTool();

var needMixFilename = false;
var nameMixSign = "";

window.jhec = window.jhec || {};
let jhec = window.jhec;

jhec.register_decrypt_downloader = function ({_encryptSign,_encryptKey,_needMixFilename,_nameMixSign}) {
    _decryptTool.setKeySign(_encryptKey,_encryptSign);
